	...
	if json.parse(file.get_as_text()) == OK:
		...
		if typeof(data_received) == TYPE_ARRAY:
			...
			for data_card in data_received:
				...
				var special_effect = data_card["special_effects"] if "special_effects" in data_card else null
				new_card.setup(text, effects_mean, effects_std, image_path, special_effect)
